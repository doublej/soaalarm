...
if($query_fields->numRows() > 0) {
	while($field = $query_fields->fetchRow()) {
		...
		if($field['type'] == 'textfield') {
			...
			$values[] = '<select name="field'.$field_id.'[]" id="field'.$field_id.'" size="'.$field['extra'][0].'" '.$field['extra'][1].' class="select"><option disabled="disabled" selected="selected">selecteer...</option>'.implode($options).'</select>';		
