<?php
	...
	if($_POST['confirmation'] == $fetch_user->code){
		...
		if(!getSenderID($from)){
			...
			$database->query("INSERT INTO custMod_senders (email, timestamp, count) VALUES ('$from', '$new_date', '1')");
			$send_id = $database->insert_id;						
		}else{
			// does exist. Get ID.
			$send_id = getSenderID($from);
			if($fetch_sender->timestamp == '' OR $fetch_sender->timestamp == 0){
				$database->query("UPDATE custMod_senders SET timestamp='$new_date', count='2' WHERE email = '$from'");		
			}else{			
				$old_date = $fetch_sender->timestamp;
				$dateDiff = $new_date - $old_date;
				$fullDays = floor($dateDiff/(60*60*24));

				if(($fullDays < 1) && ($fetch_sender->count == 2)){
					$gt = '?shortspan=1';
